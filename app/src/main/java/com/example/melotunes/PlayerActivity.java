...
public class PlayerActivity extends AppCompatActivity {
    ...
    private void nextBtnClicked() {
        if(mediaPlayer.isPlaying())
        {
            ...
            means repeat button is on
            so position value will be position itself!
             */
            uri = Uri.parse(listSongs.get(position).getPath());
            mediaPlayer = MediaPlayer.create(getApplicationContext(), uri);
            metaData(uri);
    ...
    private void getIntentMethod() {
        ...
        seekBar.setMax(mediaPlayer.getDuration() / 1000); //divided by 1000 so as to make it as seconds parameter
        metaData(uri);
    }

    //initialising the views used in player xml
    ...
    //takes data of songs into the Player Activity at run-time
